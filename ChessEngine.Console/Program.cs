...
// ── Divide mode ──────────────────────────────────────────────────────────────
// Uncomment to debug a specific position at a specific depth:
//
/*
 Console.WriteLine();
 Console.WriteLine("── Divide ──");
 var debugBoard = new Board("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");

// Play the diverging move first
var moveGen = new MoveGenerator();
var moves = moveGen.GenerateLegalMoves(debugBoard);
var d2d3 = moves.First(m => m.ToString() == "d2d3");
debugBoard.MakeMove(d2d3);

moves = moveGen.GenerateLegalMoves(debugBoard);
var b7b5 = moves.First(m => m.ToString() == "b7b5");
debugBoard.MakeMove(b7b5);

moves = moveGen.GenerateLegalMoves(debugBoard);
var e1d2 = moves.First(m => m.ToString() == "e1d2");
debugBoard.MakeMove(e1d2);

moves = moveGen.GenerateLegalMoves(debugBoard);
var b5b4 = moves.First(m => m.ToString() == "b5b4");
debugBoard.MakeMove(b5b4);

d2c3

Console.WriteLine($"FEN after d2d3: {debugBoard.ToFEN()}");
perft.Divide(debugBoard, 1);
*/